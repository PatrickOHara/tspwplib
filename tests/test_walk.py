...
from tspwplib import VertexFunctionName, edge_list_from_walk, is_simple_cycle, is_simple_path, is_walk, total_cost, total_cost_graph_tool, total_cost_networkx, total_prize
...
def test_total_cost_networkx(weighted_walk_graph):
    """Test total cost"""
    assert total_cost_networkx(weighted_walk_graph, [0, 1, 2]) == 1 + 3
    assert total_cost_networkx(weighted_walk_graph, []) == 0
    assert total_cost_networkx(weighted_walk_graph, [0, 1, 2, 1, 0]) == 1 + 3 + 3 + 1
    with pytest.raises(KeyError):
        total_cost_networkx(weighted_walk_graph, [0, 1, 2, 3])

def test_total_prize(weighted_walk_graph):
    """Test total prize"""
    prizes = nx.get_node_attributes(weighted_walk_graph, VertexFunctionName.prize)
    assert total_prize(prizes, [0, 1, 2]) == 3
    assert total_prize(prizes, []) == 0

def test_total_cost_graph_tool(weighted_walk_graph):
    """Test total cost"""
    pass
